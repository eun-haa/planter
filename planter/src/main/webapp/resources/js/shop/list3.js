...
$(document).ready(function(){
	...
	$.getJSON("/shop/prodInfoList" + ".json", function(list){
		...
		$.getJSON("/shop/prodFileList" + ".json", function(data){// ShopController에 있는 fileList를 통해 얻어진 select 결과를 data에 저장한 후,
			...
			$(list).each(function(i1,obj1){
				...
				$(data).each(function(i,obj){ // data : select된 결과
					...
					if(list[i1].pno == data[i].pno){
						...
						// 사용자가 업로드 한 파일의 타입이 이미지이면(jpg,png,gif) img 태그를 사용해서 웹브라우저에 이미지 출력
